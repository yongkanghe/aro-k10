...
PASSWORD=$(az aro list-credentials --name $MY_CLUSTER --resource-group $MY_GROUP -o tsv --query kubeadminPassword)

apiServer=$(az aro show -g $MY_GROUP -n $MY_CLUSTER --query apiserverProfile.url -o tsv)
